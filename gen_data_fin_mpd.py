...
def main():
    ...
    sequenced_data_dir = r'C:\Users\aibpo\OneDrive\Documents\GitHub\mpd-music-recommender-evaluation\data\sequenced'
    train_file_path = os.path.join(sequenced_data_dir, 'mpd_train.json')
    test_file_path = os.path.join(sequenced_data_dir, 'mpd_test.json')

    # Validate file existence
    if not os.path.exists(train_file_path):
        print(f"Train file {train_file_path} does not exist.")
        exit(1)
    ...
    with open(train_file_path, 'r') as train_file:
        user_train_data = json.load(train_file)

    with open(test_file_path, 'r') as test_file:
        user_test_data = json.load(test_file)

    # Initialize RNG and vocabulary
    rng = random.Random(random_seed)
    vocab = FreqVocab(user_test_data)

    # Convert test data to vocab indices
    ...
    output_filename = os.path.join(output_dir, f"{dataset_name}{version_id}.train.tfrecord")
    gen_samples(
        user_train_data,
        output_filename,
        rng,
    ...
    print('train:', output_filename)

    # Generate test data
    print('begin to generate test')
    output_filename = os.path.join(output_dir, f"{dataset_name}{version_id}.test.tfrecord")
    gen_samples(
        user_test_data,
        output_filename,
        rng,
    ...
    print('test:', output_filename)

    # Save vocab and history files
    ...
    vocab_file_name = os.path.join(output_dir, f"{dataset_name}{version_id}.vocab")
    ...
    his_file_name = os.path.join(output_dir, f"{dataset_name}{version_id}.his")
